	...
	protected function _prepareFilterStatements(DatabaseListHelperFilter $filter){
		...
		if($filter->count() > 0){
			...
			if($username = $filter->get(User::FIELD_USERNAME)){
				$filters['where'] .= 'AND `'.User::FIELD_USERNAME.'` LIKE \''.$this->escapeString($username).'\'';
			}
			if($email = $filter->get(User::FIELD_EMAIL)){
				$filters['where'] .= 'AND `'.User::FIELD_EMAIL.'` LIKE \''.$this->escapeString($email).'\'';
			}
			if(!is_null($activated = $filter->get(User::FIELD_ACTIVATED))){
				$filters['where'] .= 'AND `'.User::FIELD_ACTIVATED.'` LIKE '.MySQLiTools::parseBooleanValue($activated);
			}
			if(!is_null($deleted = $filter->get(User::FIELD_DELETED))){
				$filters['where'] .= 'AND `'.User::FIELD_DELETED.'` LIKE '.MySQLiTools::parseBooleanValue($deleted);
			}
