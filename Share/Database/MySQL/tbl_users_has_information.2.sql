CREATE TABLE IF NOT EXISTS `tbl_users_has_information` (
...
ALTER TABLE `tbl_users_has_information`
  ADD CONSTRAINT `tbl_users_has_information_ibfk_3` FOREIGN KEY (`fk_information_items`) REFERENCES `tbl_information_items` (`pk_information_items`) ON DELETE CASCADE ON UPDATE CASCADE,
